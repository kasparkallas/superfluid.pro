...
/**
 * GET /superfluid.tokenlist.json
 * Returns a tokenlist containing only mainnet listed tokens (excludes testnet tokens)
...
export const GET = async (request: Request) => {
	try {
		...
		// - isListed=true: Only include listed tokens
		// - excludeTags=testnet: Exclude tokens tagged as testnet
		const tokenlistUrl = new URL("/tokenlist", baseUrl)
		tokenlistUrl.searchParams.set("isListed", "true")
		tokenlistUrl.searchParams.set("excludeTags", "testnet")
		...
		console.error("Failed to fetch mainnet tokenlist:", error)
