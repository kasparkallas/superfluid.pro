...
```bash
npm install @sfpro/sdk @wagmi/core viem hono
```
...
import { readSuperToken, readCfaForwarder } from "@sfpro/sdk/action"
...
```ts title="pools.ts"
import { Hono } from 'hono'
import { readGdaPool, readGdaForwarder } from "@sfpro/sdk/action"
